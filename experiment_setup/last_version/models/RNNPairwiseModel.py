...
class RNNPairwiseModel:
    ...
    def __init__(self, w2v, context_window_sz = 20, dropout = 0.0,
        ...
        if self._feature_generator is not None:
            x = merge([left_lstm, right_lstm, mention_input_n, candidate1_input_n, candidate2_input_n,
                       extra_features_input_n], mode='concat')
        else:
            x = merge([left_lstm, right_lstm, mention_input_n, candidate1_input_n, candidate2_input_n], mode='concat')
        ...
        if self._feature_generator is not None:
            model = Model(input=[left_context_input, right_context_input, mention_input, candidate1_input_n,
                                 candidate2_input_n, extra_features_input_n], output=[out])
        else:
            model = Model(input=[left_context_input, right_context_input, mention_input, candidate1_input_n,
                                 candidate2_input_n], output=[out])
    ...
    def predict(self, wikilink, candidate1, candidate2, return_score = False):
        vecs = self._2vec(wikilink, candidate1, candidate2)
        if not isinstance(vecs, tuple) and return_score:
            return vecs, 1.0 # in case where one of the candidates was not found
        elif not isinstance(vecs, tuple):
            return vecs
        (left_X, right_X, mention_X, candidate1_X, candidate2_X, extra_features_X) = vecs
        ...
        X = {'left_context_input': left_X.reshape(1,left_X.shape[0],left_X.shape[1]),
             ...
             'candidate1_input': candidate1_X.reshape(1,candidate1_X.shape[0]),
             'candidate2_input': candidate2_X.reshape(1,candidate2_X.shape[0])}
        if self._feature_generator is not None:
            X['extra_features_input'] = np.array(extra_features_X.reshape(1,extra_features_X.shape[0]))

        Y = self.model.predict(X, batch_size=1)
        if return_score:
            winner_prob_candi1 =  Y[0][0]/(Y[0][0] + Y[0][1]) if Y[0][0] > Y[0][1] else 1 - Y[0][1]/(Y[0][0] + Y[0][1])
            winner = candidate1 if Y[0][0] > Y[0][1] else candidate2
            return winner, winner_prob_candi1
        else:
