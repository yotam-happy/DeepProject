...
class RNNPointwiseModel:
    ...
    def __init__(self, w2v, context_window_sz = 20, dropout = 0.0,
        ...
        candidate_input = Input(shape=(self._w2v.conceptEmbeddingsSz,), name='candidate_input')
        if self._feature_generator is not None:
            extra_features_input = Input(shape=(self._feature_generator.numPointwiseFeatures(),), name='extra_features_input')
        ...
        if noise is not None:
            ...
            candidate_input_n = GaussianNoise(noise)(candidate_input)
            if self._feature_generator is not None:
            ...
            candidate_input_n = candidate_input
            if self._feature_generator is not None:
        ...
        if self._feature_generator is not None:
            x = merge([left_lstm, right_lstm, mention_input_n, candidate_input_n,
                       extra_features_input_n], mode='concat')
        else:
            x = merge([left_lstm, right_lstm, mention_input_n, candidate_input_n], mode='concat')
        ...
        if self._feature_generator is not None:
            model = Model(input=[left_context_input, right_context_input, mention_input, candidate_input_n,
                                 extra_features_input_n], output=[out])
        else:
            model = Model(input=[left_context_input, right_context_input, mention_input, candidate_input_n],
                          output=[out])
    ...
    def _2vec(self, wikilink, candidate):
        """
        ...
        if candidate not in self._w2v.conceptDict:
            return None
        if 'right_context' not in wikilink and 'left_context' not in wikilink:
            return None

        candidate_vec = self._w2v.conceptEmbeddings[self._w2v.conceptDict[candidate]]
        extraFeatures_vec = np.array(self._feature_generator.getPointwiseFeatures(wikilink, candidate)) \
        ...
        return left_context, right_context, mention_vec, candidate_vec, extraFeatures_vec
    ...
    def train(self, wikilink, candidate, correct):
        """
        ...
        """
        vecs = self._2vec(wikilink, candidate)
        if not isinstance(vecs, tuple):
            return # nothing to train on

        (left_X, right_X, mention_X, candidate_X, extra_features_X) = vecs
        Y = np.array([1, 0] if candidate == correct else [0, 1])
        # Check for nan
        if np.isnan(np.sum(left_X)) or np.isnan(np.sum(right_X)) \
                or np.isnan(np.sum(candidate_X))  or np.isnan(np.sum(mention_X)):
            print "Input has NaN, ignoring..."
            return
        self._trainXY(left_X, right_X, mention_X, candidate_X, extra_features_X, Y)

    def _trainXY(self, left_X, right_X, mention_X, candidate_X, extra_features_X, Y):
        ...
        if len(self._batchY) >= self._batch_size:
            ...
            batchX = {'left_context_input': np.array(self._batch_left_X),
                      ...
                      'candidate_input': np.array(self._batch_candidate_X)}
            if self._feature_generator is not None:
    ...
    def predict(self, wikilink, candidate):
        vecs = self._2vec(wikilink, candidate)
        if not isinstance(vecs, tuple):
            return vecs
        (left_X, right_X, mention_X, candidate_X, extra_features_X) = vecs
        ...
        X = {'left_context_input': left_X.reshape(1,left_X.shape[0],left_X.shape[1]),
             'right_context_input': right_X.reshape(1,right_X.shape[0],right_X.shape[1]),
             'mention_input': mention_X.reshape(1,mention_X.shape[0]),
             'candidate_input': candidate_X.reshape(1,candidate_X.shape[0])}
        if self._feature_generator is not None:
            X['extra_features_input'] = np.array(extra_features_X.reshape(1,extra_features_X.shape[0]))
