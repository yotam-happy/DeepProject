...
\begin{document}
	...
	In this paper, we train a Recurrent Neural Network (RNN) model, which unlike CNNs and embeddings, are naturally adapt to exploit the sequential structure of text. Moreover, we implement a neural attention mechanism, inspired by results from \newcite{Lazic2015} that used a probabilistic attention-like model and showed only a few context words have value in disambiguating a mention.
	...
	We randomly sampled and manually analyzed $200$ individual cases of prediction errors made by our model. This set was obtained from a WikilinksNED's validation set that was not used for training. 
	
	Working with crowd-sourced data, we expected some errors to result from noise in the ground truths themselves. Indeed, we found that $19.5$\% (39/200) of the prediction errors were not false, where two thirds originated from wrong ground truth annotations of the author and one third were predictions with an equivalent meaning as the correct entity. Interestingly, in $92$\% of the ground truth errors, our model suggested a more convincing solution than the original author by using specific hints from the context. In this manner, the  mention \textit{'Supreme leader'} , which was contextually associated to the Iranian leader Ali Khamenei, was linked by our model with \textit{'supreme leader of Iran'} while the "correct" tag was the general \textit{'supreme leader'} entity.
