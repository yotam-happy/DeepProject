...
We present WikilinksNED, a large-scale NED dataset of short web-page fragments. 
in contrast to existing news-based datasets WikilinksNED is significantly noisier and less coherent, making it more difficult to disambiguate. 
...
We evaluate both on WikilinksNED and on a standard, smaller, news-based dataset and find our model significantly outperforms existing state-of-the-art methods on WikilinksNED while achieving reasonable performance on the smaller dataset. 
\end{abstract}
...
We note that preliminary experiments revealed that corrupt-sampling according to the distribution of entities in the dataset (as is done by Mikolov at el. \shortcite{mikolov2013distributed}), rather then uniform sampling, produces an interesting entity-context similarity measure. However, it does not perform well in our settings due to the lack of biasing toward popular entities.
...
\begin{itemize}
	\item PPRforNED - Pershina at el. \shortcite{pershina2015personalized}
...
\toin{Omer: Error Analysis needs a table with an overview of how the errors are distributed. Don't worry about the space.}
...
Working with crowd-labeled data, we expected some of the mentions to be associated with wrong Wikipedia titles. Accordingly, we found out that $19$\% (38/200) of the error-labeled predictions were not false, where $66$\% of the mistakes originated in wrong annotations of the author and $34$\% were predictions with an equivalent meaning as the correct entity. %For instance, the music software \textit{Abelton} was labeled under the entity \textit{Abelton live}, a specific version of the program, but assigning it the \textit{Abelton} entity should not be considered an error.

Interestingly, in $92$\% (23/25) of the annotation error events, the model suggested a more convincing solution than the original author by using specific hints from the context. In this manner, the  mention \textit{'Supreme leader'} , which was contextually associated to the Iranian leader Ali Khamenei, was linked by our model with \textit{'supreme leader of Iran'} while the "correct" tag was the general \textit{'supreme leader'} entity.
...
\begin{table}[h]
	\begin{center}
		\begin{tabular}{|c| p{1.5cm}|}
			...
			\hline \bf Error type & \bf Fraction  \\ \hline
...
\toin{Omer:For examples like movie/book - how many of these can be disambiguated consistently by humans? You could argue that some cases are too ambiguous for humans to solve (very common in semantic tasks).}

\section{Conclusions}
We believe these results demonstrate that our ARNN model is better then existing state-of-the-art techniques at modeling noisy context when sufficient amounts of training examples are available. This allows us to significantly outperform state-of-the-art algorithms which are found to be suboptimal in a noisy environment such as web content. 
...
\toin{Omer: Disambiguation pages should be filtered out of the candidate generation process. Mention this as something to fix in future work.
}
