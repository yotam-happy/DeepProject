...
\author{
	Yotam Eshel $^{1}$
	\qquad Noam Cohen$^{1}$
	\qquad Kira Radinsky$^{1,2}$ 
	\qquad Shaul Markovitch$^{1}$
	\\  \textbf{Ikuda Yamada$^{3}$}
	\qquad \qquad \qquad  \textbf{Omer Levi$^{4}$}\\ \\
$^{1}$Technion  - Israel Institute of Technology, Haifa, Israel\\
\texttt{\{yotamesh, shaulm, kirar\}@cs.technion.ac.il} \\
\texttt{noambox@gmail.ccom} \\
$^{2}$eBay Research\\
$^{3}$Studio Ousia 4489-105-221 Endo, Fujisawa, Kanagawa, Japan\\
\texttt{ikuya@ousia.jp} \\
$^{4}$Paul G. Allen School, University of Washington, Seattle, WA\\
