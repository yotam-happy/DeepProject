...
\begin{document}
	...
	In this work, we investigate the task of NED in a setting where only local and noisy context is available. Such settings are common in domains such as web page fragments, social media, or questions, where text is often short, noisy and less coherent. Due to the shortness and lack of coherence, global algorithms are difficult to apply in this setting. For example \newcite{Huang2014Collective} requires disambiguating many tweets together for applying a global model. Local algorithms must be able to handle short and noisy text as well. Take for example this fragment taken from the web:
	...
	In this work we create a dataset of 3.2M short text fragments extracted from web pages, each containing a mention of a named entity. Our dataset contains 18K unique mentions linking to over 100K unique entities. This dataset is two orders of magnitude larger than previously collected datasets, and we have empirically found it to be significantly noisier and more challenging than standard datasets.
