...
\paragraph{Training} we sample candidates using Near-Misses which was found to perform well due to a large training data that represents the test-data well.
...
Comparison of All-Entity and Near-Misses corrupt-sampling reveals that when using when Near-Misses, our model achieves significantly improved performance. We attribute this difference both to the more efficient nature of training with near misses, and to Near-Misses preserving the conditional-prior of entities which is a stronger signal then the prior probability preserved by All-Entity corrupt-sampling.
