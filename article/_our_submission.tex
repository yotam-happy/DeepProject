...
Sampling from all possible entities rather then only possible candidates for each mention was done for two reasons: it diversifies the contexts in which each entity is given to the model as a negative example allowing the model to better generalize for unseen mentions and contexts. It also allows to utilize unambiguous examples where only single candidate is found (i.e. Wikipedia cross-references used in section \ref{experiments-conll} which contain a great number of unambiguous mentions) \todo{YOTAM: is this explanation good?}
...
In our experiments we trained all models for $20$ epochs mentions and $k=5$ for corrupt example generation. We found the performance of our model substantially improves for the first few epochs and then continues to slowly converge with marginal gains. \todo{YOTAM - I have the training graph, here or in the evaluation section? }
...
In all experiments, our model was trained with fixed-size left and right contexts (20 words in each direction). We used a special padding symbol when the actual context was shorter than the window. Further, we filtered stopwords using NLTK's stop-word list prior to selecting the window in order to focus on more informative words.\todo{Before or after selecting the window? If the stopwords are thrown before, the window is effectively larger. Also, why was this necessary? After - Focus on more informative words. In hindsight i'm not sure this was necessary}
