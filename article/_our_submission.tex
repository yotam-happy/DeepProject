...
\begin{document}
	...
	\toin{Omer: How about the following instead of the "We differ..." sentence:
	While RNNs (CITE NED WORK WITH RNNS) and attention (CITE NED WORK WITH ATTENTION) have been applied separately to the task, this is the first model to combine the two architectures for NED.
	}
	We differ from existing neural-based approaches by accounting for the sequential nature of textual context using RNNs, and using an attention model to reduce the impact of noise in the text. 
	We train our model with a novel method for sampling negative examples and initialize it with a new kind of word and entity embeddings, both of which substantially improve its performance.
