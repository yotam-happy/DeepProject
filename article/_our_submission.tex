...
The first attempt to use Wikipedia as an knowledge base for local disambiguation was offered by Busco and Pasca back in 2006 \cite{Bunescu2006}. Hoffart et al. suggested a collective global disambiguation graph-based framework, named AIDA, which employed mention, context and coherency models \cite{hoffart2011robust}. Similar models were exploited in other global algorithms, such as the GLOW and the Relational Inference systems for the task of Wikification \cite{Ratinov2011,Cheng2013}. Chisholm et al. incorporated web-links data from the Wikilinks dataset \cite{Singh2012} to push NED performance up to a new level \cite{chisholm2015entity}. More recently, a selective-context model, proposed by Lazic et al., has shown that only few context words are informative for the tasks of disambiguation \cite{Lazic2015}.
...
In this paper, we embed both words and Wikipedia entities in the same space to form input vectors for a Recurrent Neural Network (RNN) model. In this manner, we not only exploit the sequential structure of local surrounding context, but incorporate semantic world knowledge from a rich and broad corpus in our representation of text. Our novel solution combines these properties with an attention module, thus enables  to demonstrate state-of-the-art local NED results on the several disambiguation tasks.

\subsection{Datasets for NED}

One of the most commonly used evaluation for benchmarking the NED challenge \shortcite{Globerson2016,Hachey2013,Yamada2016,Pershina2015} is the CoNLL corpus, which was crafted from the CoNLL-2003 Named Entity Recognition (NER) task \shortcite{hoffart2011robust}. CoNLL was composed by manually annotating Reuters newswire articles from 1996. Its contains $1393$ documents from a period of $12$ days split into train, development and test sets. Following previous works we have only evaluated our method on non-NIL mentions.

TAC KBP is another popular dataset for testing the performance of disambiguation systems \cite{Chisholm2015,Globerson2016,sun2015modeling}. Similar to CoNLL, it is primarily based on news articles and is specifically designed for the task of entity linking in the Text Analysis Conference (TAC)\cite{ji2010overview}. The most broadly used version for NED is the TAC 2010 data set, which includes a training and test set of $1,070$ and $1,017$ non-NIL mentions, respectively. It is important to note that this data set was not available to us, as it is distributed only to TAC participants.
There are also some disambiguation experiments  that are evaluated on the ACE 2005 corpus \cite{Ratinov2011,Francis-Landau2016}. Being composed out of news reports from various sources, this corpus was extend by connecting and annotating it using Wikipedia \cite{bentivogli2010extending}. It features $16,310$ annotated mentions when only $1,458$ of them have multiple links, hence making them challenging for NED. 
