...
\begin{document}
	...
	\toin{General English comment: you use the "have been" been tense quite a bit. Prefer past simple. E.g: we ran the model, we crawled the web, we wrote code, etc.}
	...
	\toin{I think the "left context" label should be on top and the "right context" at the bottom (other way around).}
	
	\toin{You should refer to figure 1 from within the text, in this section, as soon as possible. Consider adding an opening sentence "Figure ref illustrates..."}
	...
	\toin{Missing from this description: word embeddings (the bottom-most layer). It needs to be mentioned that the input vectors $v_t$ are embeddings, and in a footnote, their size.}
	
	\subsection{Training}
	
	We assume our model is only given examples of correct entity assignments during training and therefore use \textit{corrupt-sampling}, where we automatically generate examples of corrupt assignments. For each context-entity pair $(c,e)$, where $e$ is the correct assignment for $c$, we produce $k$ corrupt examples with the same context $c$ but with a different, corrupt entity $e'$. We considered two alternatives for corrupt sampling:
	...
	\paragraph{Results}
	We used standard micro P@1 accuracy for evaluation. Experimental results comparing our model with the baselines are reported in Table \ref{tab:wikilink}. Our RNN model significantly outperforms Yamada at el. on this data by over 5 points, indicating that the more expressive RNNs are indeed beneficial for this task. We find that the attention mechanism further improves our results by a small margin.
	...
	\paragraph{Training}
	CoNLL-YAGO has a training set with $18505$ non-NIL mentions, which preliminary experiments showed is not sufficient to train our model on. We therefore resorted to domain adaptation for training. We first trained our model on a large corpus of Wikipedia derived data, and adapted the trained model to CoNLL-YAGO using its training set. We proceeded to use the model in a similar setting to \newcite{yamada2016joint} where a GBRT was trained with our model as a feature along with the statistical and string based features defined by Yamada.\todo{This is the first mention of GBRT. WTF is a GBRT? :)} 
