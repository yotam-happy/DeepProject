...
\begin{document}
	...
	In this work, we investigate the task of NED in a setting where only local and noisy context is available. Such settings are common in domains such as web page fragments, social media, or questions, where text is often short, noisy and less coherent. Due to the shortness and lack of coherence, global algorithms cannot be directly applied to such setting without relying on some extra knowledge (CITE). Local algorithms must be able to handle short and noisy text as well. Take for example this fragment taken from the web:
