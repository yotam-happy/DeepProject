...
** Cucerzan (2007) \toin{should we talk about it???!}
 
** Bunesco (2006)
The first attempt to use Wikipedia as an knowledge base for local disambiguation was offered by Busco and Pasca and included an SVM based NED system with local contextual features \cite{Bunescu2006}.

** Hoffart (2011)
Hoffart et al. suggested a collective disambiguation graph-based framework called AIDA, which employs a mention, context and coherency models. \cite{hoffart2011robust}. \hl{talks about the problem with coherence can when entities are forced to be coherent in the doc cluster. claims that in his experiments $2/3$ of the mentions didn't even proceed to the coherence graph module}

** Ratinov and Cheng (2011 and 2013)
Similar models were exploited in other global algorithms, such as GLOW and Relational Inference for the task of Wikification \cite{Ratinov2011,Cheng2013}, when the latter extends the former approach by combing inference with semantic knowledge and a ranking SVM model. \hl{Ratinov shows that local based methods are hard to be beat}
