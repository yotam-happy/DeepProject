...
\begin{document}
	...
	Current research on NED is mostly driven by a number of standard datasets, such as CoNLL-YAGO \cite{hoffart2011robust}, TAC KBP \cite{ji2010overview} and ACE \cite{bentivogli2010extending}. These datasets are based on news corpora and Wikipedia, which are naturally coherent, well-structured, and rich in context. A wide variety of algorithms were developed to address these datasets \cite{Shen2015Entity}. In particular global algorithms, which employ a coherency model to disambiguate all mentions within a document together were shown to outperform local approaches that disambiguate each mention separately \cite{guo2014entity,pershina2015personalized,Globerson2016}. However, domains such as web page fragments, social media, or questions, are often short, noisy and less coherent. Such domains lack the necessary context and coherence required for global methods to pay off and present a more challenging setting for local methods.
	...
	However, global approaches are difficult to apply in domains where only short and noisy text is available, as occurs often in social media, questions and answers, and other short web documents. For example, \newcite{Huang2014Collective} requires disambiguating many tweets of the same author together for applying a global model. Our proposed model is a local model which disambiguates each mention separately since in our setting the short web-fragments lack sufficient context and coherency for global coherency-models.
	...
	In our work we find noisy web data to be a unique and challenging test case for disambiguation and therefore use Wikilinks to construct a noisy test case in-itself, rather than use it for training alone. 
	...
	\begin{table}[ht]
		\begin{center}
			\begin{tabular}{|p{3.5cm}| p{1.3cm} p{1.3cm}|}
				\hline \multicolumn{3}{|c|}{CoNLL-YAGO test-b - training steps evaluation} \\
